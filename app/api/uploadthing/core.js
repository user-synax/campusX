...
async function authenticateRequest(req) {
    ...
    if (!token) {
        throw new UploadThingError({
            code: "UNAUTHORIZED",
            message: "Session required for upload.",
        });
    }
...
export const ourFileRouter = {
    ...
    postImageUploader: f({
        ...
        .middleware(async ({ req }) => {
            const metadata = await authenticateRequest(req);
            // Check Pro status using existing helper
            const isPro = await refreshUserProStatus(metadata.userId);
            if (!isPro) {
                throw new UploadThingError({
                    code: "FORBIDDEN",
