...
export default function ActivityHeatmap({ username }) {
    ...
    useEffect(() => {
        ...
        fetch(`/api/users/${username}/activity`)
            ...
            .then((data) => {
                const map = {};
                for (const { date, count } of data.activity ?? [])
                    map[date] = count;
                setActivityMap(map);
            })
            .catch(() => {})
    ...
    const { grid, monthLabels, totalContributions } = useMemo(() => {
        ...
        const gridStart = startOfWeek(subDays(today, WEEKS * 7 - 1), {
            weekStartsOn: 0,
        });
        ...
        while (padded.length % 7 !== 0) {
            padded.push(
                new Date(padded[padded.length - 1].getTime() + 86400000),
            );
        }
    ...
    return (
        <div
            className="rounded-xl border border-border bg-card p-4 select-none"
            ref={containerRef}
        >
            {/* Header */}
            <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-xs text-muted-foreground">
                    {totalContributions.toLocaleString()} contributions this
                    year
                </span>
            </div>
            ...
            <div className="overflow-x-auto pb-1">
                ...
                >
                    ...
                    {[0, 1, 2, 3, 4, 5, 6].map((dayIdx) => (
                      ...
                      ${isActive ? "ring-1 ring-foreground/70 scale-125 z-10" : "hover:ring-1 hover:ring-foreground/40 hover:scale-110"}
                    `}
                                        style={{
                                            width: CELL,
                                            height: CELL,
