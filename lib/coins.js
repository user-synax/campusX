...
export const COIN_VALUES = {
  ...
  referral_bonus: 100,
  // Engagement boosters
  resource_downloaded: 3,   // someone downloaded your resource
  profile_visited: 1,   // someone visited your profile (capped by daily limit)
  streak_3day: 15,  // 3-day login streak bonus
  post_shared: 8,   // sharing a post externally
  event_attended: 20,  // marked attendance at an event
  helpful_answer: 15,  // comment marked as helpful by OP
}
...
const EQUIP_FIELD_MAP = {
  ...
  entry_effect: 'equipped.entryEffect'
}
...
export async function equipItem(userId, itemSlug) {
  ...
  // Save item with slug and flatten visual data for components that expect properties directly
  const equippedData = {
    slug: item.slug,
    name: item.name,
    ...item.visual
  ...
  await Wallet.findOneAndUpdate(
    { userId },
    { $set: { [field]: equippedData } }
  )

  return { success: true, slot: field.split('.')[1], equipped: equippedData }
}
...
export async function unequipSlot(userId, slot) {
  ...
  const validSlots = [
    ...
    'postBadge', 'chatBubble', 'bioTheme', 'profileTheme', 'effect', 'entryEffect'
  ]
...
export async function getWalletData(userId, options = {}) {
  await connectDB()
  ...
  const equippedSlugs = Object.values(wallet.equipped || {}).filter(Boolean)
  ...
  let inventory = []
  if (options.includeInventory && wallet.inventory?.length > 0) {
    const itemIds = wallet.inventory.map(i => i.itemId)
    inventory = await ShopItem.find({ _id: { $in: itemIds } }).lean()

    // Mark as owned for consistent object structure with shop API
    inventory = inventory.map(item => ({ ...item, isOwned: true }))
  }
  ...
  return {
    ...
    inventory: options.includeInventory ? inventory : undefined
  }
}
