...
export async function attachEquippedToItems(items, authorPath = 'author') { 
  ...
  Object.values(walletMap).forEach(equipped => {
    ...
    Object.values(e).forEach(val => {
      if (val) {
        // New format: { slug: 'xxx', name: '...', visual: {...} }
        if (typeof val === 'object' && val.slug) {
          allSlugs.add(val.slug)
        }
        // Legacy format: 'xxx' (just slug string)
  ...
  for (const authorId of authorIds) { 
    ...
    const e = equipped.toObject ? equipped.toObject() : equipped
    const resolved = {}
    for (const [slot, val] of Object.entries(e)) {
      if (!val) continue

      // Get the slug from either object format or string format
      const slug = typeof val === 'object' ? val.slug : val

      if (slug && shopVisuals[slug]) {
        // Use existing object format if available, otherwise create from shop visual
        ...
        // If no shop visual found, keep original value
        resolved[slot] = typeof val === 'object' ? val : { slug }
      }
    }
